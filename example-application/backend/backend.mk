...
export VEGITO_EXAMPLE_APPLICATION_BACKEND_IMAGES_BASE ?= $(VEGITO_EXAMPLE_APPLICATION_PUBLIC_IMAGES_BASE):backend
export VEGITO_EXAMPLE_APPLICATION_BACKEND_IMAGE       ?= $(VEGITO_EXAMPLE_APPLICATION_BACKEND_IMAGES_BASE)-$(VERSION)
export VEGITO_EXAMPLE_APPLICATION_BACKEND_DIR         ?= $(VEGITO_EXAMPLE_APPLICATION_DIR)/backend
export VEGITO_EXAMPLE_APPLICATION_BACKEND_INSTALL_BIN ?= $(HOME)/go/bin/backend
export VEGITO_EXAMPLE_APPLICATION_BACKEND_VENDOR      ?= $(VEGITO_EXAMPLE_APPLICATION_BACKEND_DIR)/vendor
...
example-application-backend-install:
	...
	cd $(VEGITO_EXAMPLE_APPLICATION_BACKEND_DIR) \
	  && CGO_ENABLED=1 go install -a -ldflags "-linkmode external -extldflags -static"
