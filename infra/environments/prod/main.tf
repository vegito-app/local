...
module "kubernetes" {
  ...
  vegetable_image_created_moderator_pull_pubsub_subscription = google_pubsub_subscription.vegetable_created_moderator_pull_subscription.name
  ...
  created_input_bucket = google_storage_bucket.firebase_storage_bucket.name
  # The bucket where the images are stored after moderation
  validated_output_bucket = module.cdn.google_storage_bucket.public_images_bucket.name
...
resource "google_pubsub_subscription" "vegetable_created_moderator_pull_subscription" {
  provider = google-beta.no_user_project_override
  project  = var.project_id
...
resource "google_pubsub_subscription" "vegetable_validated_push_subscription" {
  provider = google-beta.no_user_project_override
  project  = var.project_id
  name     = "vegetable-validated-subscription"
  topic    = google_pubsub_topic.application_images_validated.id
  push_config {
    push_endpoint = "${module.application.application_backend_url}/internal/vegetable"
    oidc_token {
      service_account_email = module.application.application_backend_cloud_run_sa_email
    }
  }
  ack_deadline_seconds       = 60
  message_retention_duration = "604800s" # 7 days
...
resource "google_storage_bucket_iam_member" "input_images_workers" {
  for_each = toset([
