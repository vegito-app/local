...
terraform-secrets-init: $(GOOGLE_CLOUD_APPLICATION_CREDENTIALS)
	@$(SECRETS_TERRAFORM) init --upgrade
.PHONY: terraform-secrets-init

terraform-secrets-import: $(GOOGLE_CLOUD_APPLICATION_CREDENTIALS)
...
terraform-secrets-state-rm: $(GOOGLE_CLOUD_APPLICATION_CREDENTIALS)
	@$(SECRETS_TERRAFORM) state rm module.infra.google_firebase_database_instance.utrade
.PHONY: terraform-secrets-state-rm

terraform-secrets-state-list: $(GOOGLE_CLOUD_APPLICATION_CREDENTIALS)
...
$(SECRETS_TF_STATE_ITEMS:%=%-show): $(GOOGLE_CLOUD_APPLICATION_CREDENTIALS)
	@$(SECRETS_TERRAFORM) state show $(@:%-show=%)
.PHONY: $(SECRETS_TF_STATE_ITEMS:%=%-show)

$(SECRETS_TF_STATE_ITEMS:%=%-taint): $(GOOGLE_CLOUD_APPLICATION_CREDENTIALS)
...
terraform-secrets-upgrade: $(GOOGLE_CLOUD_APPLICATION_CREDENTIALS)
	@$(SECRETS_TERRAFORM) init -upgrade
.PHONY: terraform-secrets-upgrade

terraform-secrets-reconfigure: $(GOOGLE_CLOUD_APPLICATION_CREDENTIALS)
	@$(SECRETS_TERRAFORM) init -reconfigure
.PHONY: terraform-secrets-reconfigure

terraform-secrets-plan: $(GOOGLE_CLOUD_APPLICATION_CREDENTIALS)
	$(SECRETS_TERRAFORM) plan -out=$(TERRAFORM_SECRETS_ROOT_MODULE)/.planed_terraform
.PHONY: terraform-secrets-plan

terraform-secrets-unlock: $(GOOGLE_CLOUD_APPLICATION_CREDENTIALS)
	@$(SECRETS_TERRAFORM) force-unlock $(LOCK_ID)
.PHONY: terraform-secrets-unlock

terraform-secrets-providers: $(GOOGLE_CLOUD_APPLICATION_CREDENTIALS)
	@$(SECRETS_TERRAFORM) providers -v
.PHONY: terraform-secrets-providers

terraform-secrets-validate: $(GOOGLE_CLOUD_APPLICATION_CREDENTIALS)
	@$(SECRETS_TERRAFORM) validate
.PHONY: terraform-secrets-validate

terraform-secrets-refresh: $(GOOGLE_CLOUD_APPLICATION_CREDENTIALS)
	@$(SECRETS_TERRAFORM) refresh
.PHONY: terraform-secrets-refresh

terraform-secrets-apply-auto-approve: $(GOOGLE_CLOUD_APPLICATION_CREDENTIALS)
...
terraform-secrets-output: $(GOOGLE_CLOUD_APPLICATION_CREDENTIALS)
	@$(SECRETS_TERRAFORM) output -json
.PHONY: terraform-secrets-output

terraform-secrets-destroy: $(GOOGLE_CLOUD_APPLICATION_CREDENTIALS)
	@$(SECRETS_TERRAFORM) destroy
.PHONY: terraform-secrets-destroy

terraform-secrets-state-backup: $(GOOGLE_CLOUD_APPLICATION_CREDENTIALS)
