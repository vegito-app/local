...
expose-port() {
  # Usage: expose-port <container_name> <host_port> <container_port>
  if [ $# -ne 3 ]; then
    echo "Usage: expose-port <container_name> <host_port> <container_port>"
    return 1
  fi
  ...
  docker run --rm -s-network $NETWORK alpine/socat \
