...
target "vegito-debian-flutter-base" {
  ...
  context = VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_DIR
}
...
# ###################################################################
variable "VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_DIR" {
  default = "${VEGITO_DOCKER_DIR}/debian/flutter"
}

variable "VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_REGISTRY_CACHE_IMAGE" {
  default = "${VEGITO_CACHE_IMAGES_BASE}/debian-flutter"
}

variable "VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_REGISTRY_CACHE" {
  default = "${VEGITO_CACHE_IMAGES_BASE}/debian-flutter"
}
variable "VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_VERSION" {
  default = "${VEGITO_DOCKER_PUBLIC_IMAGES_BASE_NAME}:debian-flutter-${VERSION}"
}

variable "VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_LATEST" {
  default = "${VEGITO_DOCKER_PUBLIC_IMAGES_BASE_NAME}:debian-flutter-latest"
}

variable "VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_DOCKER_BUILDX_LOCAL_CACHE_VERSION" {
  default = "${VEGITO_DOCKER_BUILDX_LOCAL_CACHE_DIR}/debian-flutter-version"
}

variable "VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_DOCKER_BUILDX_LOCAL_CACHE_LATEST" {
  default = "${VEGITO_DOCKER_BUILDX_LOCAL_CACHE_DIR}/debian-flutter-latest"
}

variable "VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_DOCKER_BUILDX_CACHE_WRITE_VERSION" {
  description = "local write cache (version) for debian-flutter image build"
  default     = "type=local,mode=max,dest=${VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_DOCKER_BUILDX_LOCAL_CACHE_VERSION}"
}

variable "VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_DOCKER_BUILDX_CACHE_WRITE_LATEST" {
  description = "local write cache (latest) for debian-flutter image build"
  default     = "type=local,mode=max,dest=${VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_DOCKER_BUILDX_LOCAL_CACHE_LATEST}"
}

variable "VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_DOCKER_BUILDX_LOCAL_CACHE_READ_VERSION" {
  description = "local read cache (version)"
  default     = "type=local,src=${VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_DOCKER_BUILDX_LOCAL_CACHE_VERSION}"
}

variable "VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_DOCKER_BUILDX_LOCAL_CACHE_READ_LATEST" {
  description = "local read cache (latest)"
  default     = "type=local,src=${VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_DOCKER_BUILDX_LOCAL_CACHE_LATEST}"
}
...
target "vegito-debian-flutter-latest-ci" {
  ...
  tags = [
    VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_LATEST,
  ]
  cache-from = concat(
    USE_REGISTRY_CACHE ? [
      "type=registry,ref=${VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_REGISTRY_CACHE}",
    ] : [],
    ENABLE_LOCAL_CACHE ? [
      VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_DOCKER_BUILDX_LOCAL_CACHE_READ_LATEST
    ] : [],
    [
      "type=inline,ref=${VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_LATEST}",
    ]
  )
  cache-to = concat(
    USE_REGISTRY_CACHE ? [
      "type=registry,ref=${VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_REGISTRY_CACHE},mode=max"
    ] : [],
    ENABLE_LOCAL_CACHE ? [
      VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_DOCKER_BUILDX_CACHE_WRITE_LATEST
    ] : [],
    [
      "type=inline"
    ]
...
target "vegito-debian-flutter-version-ci" {
  ...
  tags = [
    VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_VERSION,
  ]
  cache-from = concat(
    USE_REGISTRY_CACHE ? [
      "type=registry,ref=${VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_REGISTRY_CACHE}",
    ...
    ENABLE_LOCAL_CACHE ? [
      VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_DOCKER_BUILDX_LOCAL_CACHE_READ_VERSION
    ] : [],
    [
      "type=inline,ref=${VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_LATEST}",
    ]
  ...
  cache-to = concat(
    ENABLE_LOCAL_CACHE ? [
      VEGITO_DOCKER_DEBIAN_FLUTTER_DEBIAN_IMAGE_DOCKER_BUILDX_CACHE_WRITE_VERSION
    ] : []
  )
}
