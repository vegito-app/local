...
APPLICATION_TESTS_MOBILE_IMAGES_DIR="${1:-${APPLICATION_TESTS_MOBILE_IMAGES_DIR:-./images}}"
...
echo "📁 Chargement des images depuis : $APPLICATION_TESTS_MOBILE_IMAGES_DIR"
if [ ! -d "$APPLICATION_TESTS_MOBILE_IMAGES_DIR" ]; then
  echo "❌ Le répertoire $APPLICATION_TESTS_MOBILE_IMAGES_DIR n'existe pas."
  exit 1
fi
...
for img in "$APPLICATION_TESTS_MOBILE_IMAGES_DIR"/*.{jpg,jpeg,png}; do
  [ -e "$img" ] || continue
