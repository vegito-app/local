...
variable "LOCAL_ANDROID_APK_RUNNER_EMULATOR_IMAGE_REGISTRY_CACHE_CI" {
...
target "local-android-emulator-ci" {
  ...
  cache-from = [
    USE_REGISTRY_CACHE ? "type=registry,ref=${LOCAL_ANDROID_APK_RUNNER_EMULATOR_IMAGE_REGISTRY_CACHE_CI}" : "",
  ...
  cache-to = [
    USE_REGISTRY_CACHE ? "type=registry,ref=${LOCAL_ANDROID_APK_RUNNER_EMULATOR_IMAGE_REGISTRY_CACHE_CI},mode=max" : "type=inline"
  ]
  platforms = platforms
}
