...
COPY local/proxy /tmp/localproxy

# Add non-root user    
RUN cd /tmp/localproxy \
    && GOCACHE=/tmp/localproxy/.gocache \
    && GOPATH=/tmp/localproxy/.go \
    go install -v \
    && cp /tmp/localproxy/.go/bin/proxy /usr/local/bin/localproxy 
