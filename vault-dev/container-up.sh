...
echo "📱 Launching vault-dev-emulators compose in background..."
${LOCAL_VAULT_DEV_DIR}/docker-compose-up.sh &
compose_pid=$!
...
while :; do
  ...
  if ! kill -0 $wait_pid 2>/dev/null; then
    echo "🥳 All vault-dev-emulators ports are ready!"
    exit_code=0
    break
  fi
  sleep 1
